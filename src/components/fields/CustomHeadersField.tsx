  ...
  constructor(props: Props) {
    ...
    this.state = {
      headers: Object.keys(jsonData)
        .sort()
        .filter(key => key.startsWith('headerName'))
        .map((key, index) => {
          return {
            id: uniqueId(),
            name: jsonData[key],
  ...
  updateSettings = () => {
    ...
    const newJsonData = Object.fromEntries(
      Object.entries(this.props.dataSourceConfig.jsonData).filter(
        ([key, val]) => !key.startsWith('headerName'),
      ),
    )
    ...
    const newSecureJsonData = Object.fromEntries(
      Object.entries(this.props.dataSourceConfig.secureJsonData || {}).filter(
        ([key, val]) => !key.startsWith('headerValue'),
      ),
    )

    // then we add the current header-fields
    for (const [index, header] of headers.entries()) {
      newJsonData[`headerName${index + 1}`] = header.name
      if (!header.configured) {
        newSecureJsonData[`headerValue${index + 1}`] = header.value
      }
    }
